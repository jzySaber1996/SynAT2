...
charset = Charset()
vocab = Vocabulary()
vocab.load("../data_v0/tree_build/vocab.txt")
tag_set = Index()
tag_set.load("../data_v0/tree_build/tag2id.txt")
relation_labels = Index()
relation_labels.load('../data_v0/tree_build/relation_labels.txt')

train_data = load('../data_v0/tree_build/train.pk')
test_data = load('../data_v0/tree_build/test.pk')
...
word_embeddings = torch.tensor(np.load("../data_v0/tree_build/word2vec.vectors.npy"))
...
def train():
    ...
    for idx, batch_indices in enumerate(sampler):
        ...
        output = pack_padded_sequence(output.cpu(), lengths.cpu(), batch_first=True).data
        targets = pack_padded_sequence(targets.cpu(), lengths.cpu(), batch_first=True).data
...
def evaluate(data_groups):
    ...
    if (TP_FP+TP_FN) == 0:
        ...
        fmeasure = 2*TP/(TP_FP+TP_FN)
    return total_loss / count, pre, rec, fmeasure
